...
mod imp {
    ...
    fn read_request<R: Read>(reader: &mut R) -> Result<Request, String> {
    ...
    fn write_upload_request<W: Write>(writer: &mut W, path: &Path) -> Result<(), String> {
    ...
    fn write_response<W: Write>(writer: &mut W, status: u8, message: &str) -> Result<(), String> {
    ...
    mod tests {
        ...
        fn upload_request_round_trips_path() {
            ...
            write_upload_request(&mut bytes, &path).unwrap();
        ...
        fn response_round_trips_message() {
            ...
            write_response(&mut bytes, STATUS_OK, "queued upload /tmp/foo.md").unwrap();
