...
use crate::{
    ...
    ReflexiveObservation, RestartPortPolicy, StunMessage,
    TransactionId, TraversalAgent,
...
fn candidate(id: u32, kind: CandidateKind, addr: &str) -> Candidate {
    Candidate::with_metadata(id, 1, 1, kind, addr.parse().unwrap(), None, kind == CandidateKind::Host)
}
...
fn case_01_symmetric_symmetric_deadlock_immediate_relay() {
    ...
    let mut agent = TraversalAgent::new(
        ...
        NatKind::Symmetric,
        NatKind::Symmetric,
        vec![candidate(1, CandidateKind::ServerReflexive, "198.51.100.1:55000")],
        vec![
...
fn case_02_symmetric_to_cone_uses_inbound_peer_reflexive_port() {
    ...
    let mut cone = TraversalAgent::new(
        ...
        NatKind::Cone,
        NatKind::Symmetric,
        vec![candidate(1, CandidateKind::ServerReflexive, "198.51.100.1:5000")],
        vec![candidate(2, CandidateKind::ServerReflexive, "198.51.100.2:6000")],
    );
    assert!(cone.poll(now).iter().all(|action| !matches!(action, Action::SendStun { .. })));
...
fn case_03_eim_deviation_runs_limited_sequential_port_guessing() {
    ...
    let ports = guesses.iter().map(|candidate| candidate.addr.port()).collect::<Vec<_>>();
...
fn case_04_multilayer_nat_races_host_reflexive_and_relay_candidates() {
    ...
    assert!(actions.iter().any(|action| matches!(action, Action::UseRelay { .. })));
    assert!(actions.iter().any(|action| matches!(action, Action::SendStun { .. })));
}
...
fn case_06_no_hairpin_prefers_lan_host_path() {
    ...
    assert_eq!(first_stun(agent.poll(now)).unwrap().0, "192.168.1.3:5000".parse().unwrap());
}
...
fn case_07_ipv4_ipv6_mismatch_falls_back_to_relay() {
    ...
    let mut agent = TraversalAgent::new(
        ...
        vec![candidate(2, CandidateKind::Host, "[2001:db8::2]:5000"), relay()],
    );
    assert!(matches!(
        agent.poll(now).as_slice(),
        [Action::UseRelay {
...
fn case_13_buggy_upnp_mapping_is_only_a_verified_candidate() {
    ...
    let mut agent = TraversalAgent::new(
        ...
        NatKind::Cone,
        vec![candidate(1, CandidateKind::Host, "10.0.0.2:5000")],
        vec![candidate(2, CandidateKind::PortMapped, "198.51.100.2:40000"), relay()],
    );
    assert!(first_stun(agent.poll(now)).is_some());
    let actions = agent.poll(now + Duration::from_millis(25));
    assert!(actions.iter().all(|action| !matches!(action, Action::DirectReady { .. })));
}

#[test]
fn case_14_same_socket_metadata_survives_host_reflexive_and_relay_candidates() {
    let candidates = [
        Candidate::with_metadata(1, 7, 2, CandidateKind::Host, "10.0.0.2:5000".parse().unwrap(), None, true),
        Candidate::with_metadata(2, 7, 2, CandidateKind::ServerReflexive, "198.51.100.2:55000".parse().unwrap(), None, true),
        Candidate::with_metadata(3, 7, 2, CandidateKind::Relay, "203.0.113.10:41001".parse().unwrap(), None, true),
    ];
...
fn case_15_valid_handshake_updates_active_source_address() {
    ...
    let mut agent = TraversalAgent::new(
        ...
        NatKind::Cone,
        vec![candidate(1, CandidateKind::Host, "10.0.0.2:5000")],
        vec![candidate(2, CandidateKind::ServerReflexive, "198.51.100.2:55000")],
    );
...
fn case_21_asymmetric_latency_does_not_fail_before_minimum_deadline() {
    ...
    assert!(agent.poll(now).iter().all(|action| {
        !matches!(action, Action::UseRelay { reason: FallbackReason::DirectChecksFailed, .. })
    }));
}
...
fn case_22_packet_loss_uses_exponential_backoff_retransmits() {
    ...
    assert!(matches!(agent.poll(now).pop(), Some(Action::SendStun { retransmit: false, .. })));
    assert!(matches!(
        agent.poll(now + Duration::from_millis(100)).pop(),
        Some(Action::SendStun { retransmit: true, .. })
    ));
    assert!(matches!(
