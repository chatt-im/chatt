...
use crate::aws_lc_sys_impl::{EVP_sha256, HKDF_expand, HKDF_extract, EVP_MD};
#[cfg(feature = "all-cipher-suites")]
use crate::aws_lc_sys_impl::{EVP_sha384, EVP_sha512};
...
impl AwsLcHkdf {
    pub fn new(cipher_suite: CipherSuite) -> Option<Self> {
        let kdf = KdfId::new(cipher_suite)?;

        #[cfg(not(feature = "all-cipher-suites"))]
        if kdf != KdfId::HkdfSha256 {
            return None;
        }
...
impl AwsLcHash {
    pub fn new(cipher_suite: CipherSuite) -> Option<Self> {
        let algo = match cipher_suite {
            #[cfg(not(feature = "all-cipher-suites"))]
            CipherSuite::CURVE25519_AES128 => hmac::HMAC_SHA256,
            #[cfg(feature = "all-cipher-suites")]
