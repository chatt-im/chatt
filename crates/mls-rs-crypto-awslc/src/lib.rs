...
#[cfg(not(any(feature = "all-cipher-suites", feature = "curve25519-aes128-only")))]
compile_error!("enable either `all-cipher-suites` or `curve25519-aes128-only`");
...
use std::{mem::MaybeUninit, num::TryFromIntError};
#[cfg(feature = "all-cipher-suites")]
use std::ffi::c_int;
