...
impl<'bf> EasyWrapper<'bf> {
    pub fn new(before: &'bf str, max_width: usize) -> Result<Self> {
        ...
        let after = std::vec![0; bf_len]; // assuming default style, i.e. nobreak
    ...
    pub fn wrap_use_style<'af>(&'af mut self, style: WrapStyle) -> Result<Cow<'af, str>> {
        match style {
            ...
            WrapStyle::MayBrk(prepend_what, append_what) => {
                ...
                let prepend_str = prepend_what.unwrap_or("");
                let append_str = append_what.unwrap_or("");
                self.after.resize(
                    bf_len
                        + if prepend_str.is_empty() && append_str.is_empty() {
                            if self.before.is_ascii() {
                                bf_len / max_width
                            } else {
                                bf_len
                            }
                        } else {
                            bf_len + bf_len * prepend_str.len() + bf_len * append_str.len()
                        },
                    0,
                );
