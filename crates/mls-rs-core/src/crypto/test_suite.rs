...
pub fn verify_tests<C: CryptoProvider>(crypto: &C, signature_secret_key_compatible: bool) {
    #[cfg(any(target_arch = "wasm32", not(feature = "std")))]
    ...
    for test_suite in test_suites {
        ...
        verify_hpke_psk_tests(&cs, test_suite.hpke_psk_tests);
        ...
        verify_hash_tests(&cs, test_suite.hash_tests);
    }
}
...
fn verify_signature_tests<C: CipherSuiteProvider>(
    cs: &C,
...
fn verify_hpke_psk_tests<C: CipherSuiteProvider>(cs: &C, test_cases: Vec<HpkePskTestCase>) {
    let generated = generate_hpke_psk_tests(cs);
    verify_hpke_psk_test(cs, generated);
    verify_hpke_psk_test(cs, test_cases);
}
fn verify_hpke_psk_test<C: CipherSuiteProvider>(cs: &C, test_cases: Vec<HpkePskTestCase>) {
...
#[cfg_attr(coverage_nightly, coverage(off))]
fn generate_hpke_psk_tests<C: CipherSuiteProvider>(cs: &C) -> Vec<HpkePskTestCase> {
...
fn verify_mac_tests<C: CipherSuiteProvider>(cs: &C, test_cases: Vec<MacTestCase>) {
    for case in test_cases {
        let computed = cs.mac(&case.key, &case.data).unwrap();
...
fn verify_hash_tests<C: CipherSuiteProvider>(cs: &C, test_cases: Vec<HashTestCase>) {
    for case in test_cases {
        let computed = cs.hash(&case.input).unwrap();
...
mod hpke_rfc_conformance {
    ...
    pub fn verify_hpke_context_tests<C: TestHpke>(hpke: &C, cipher_suite: CipherSuite) {
            ...
            .filter(|tc| matches!(tc.algo.cipher_suite(), Some(c) if c == cipher_suite))
        {
            ...
            for enc_test_case in test_case.encryptions {
                ...
                // Decrypt
                let pt = context_r.open(Some(&enc_test_case.aad), &ct).unwrap();

                assert_eq!(*pt, enc_test_case.plaintext);
            }

            for test in test_case.exports {
                let exported_s = context_s.export(&test.exporter_context, test.length);
                assert_eq!(*exported_s.unwrap(), test.exported_value);

                let exported_r = context_r.export(&test.exporter_context, test.length);
                assert_eq!(*exported_r.unwrap(), test.exported_value);
            }
        }
    }
    pub fn verify_hpke_encap_tests<C: TestHpke>(hpke: &mut C, cipher_suite: CipherSuite) {
        for test_case in get_test_cases()
            .into_iter()
            .filter(|tc| matches!(tc.algo.cipher_suite(), Some(c) if c == cipher_suite))
        {
