...
impl HostTrait for Host {
    ...
    fn device_by_id(&self, id: &DeviceId) -> Option<Self::Device> {
        if id.host() != crate::platform::HostId::Alsa {
            return None;
        }

        let pcm_id = canonical_pcm_id(id.id());
        let canonical_id = DeviceId::new(id.host(), &pcm_id);
        if let Some(device) = self
            .devices()
            .ok()
            .and_then(|mut devices| devices.find(|d| d.id().ok().as_ref() == Some(&canonical_id)))
        {
            return Some(device);
        }

        Some(Device {
            desc: Some(format!("ALSA PCM {pcm_id}")),
            pcm_id,
            direction: DeviceDirection::Unknown,
            _context: self.inner.clone(),
        })
